...
public class CommandePartiellementExpediee extends Commande{

	public CommandePartiellementExpediee(int idCommande, String idClient,
			LinkedList<Produit> produits, LinkedList<Integer> quantites,
			int statut) {
	...
	public void insererCommande() {
		...
		try {
			...
			String dateLivraison = "current_date + interval '" + (r.nextInt(10) + 1) +
					"day'";
			...
			for(Colis c: colis){
				c.insererColis(this.idCommande, MOITIE_EXPEDIEE);
